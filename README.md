## # Hi there 👋, I'm SINSHI.JR

## 💻 Computer Engineering Student | Software & Systems Enthusiast

I’m passionate about building efficient systems that connect **software, networks, and hardware**.
I enjoy understanding how things work under the hood—from low-level programming to secure infrastructures.

---

## 🚀 About Me

* 🎓 Computer Engineering student
* 🧠 Interested in: AI, Operating Systems, Cybersecurity, Hardware and Embedded Systems
* 🛠️ Building projects that mix software + hardware
* 🌍 Based in Morocco

---

## 🛠️ Tech Stack

### 💻 Programming

* C++, Python, Java (learning)

### 🌐 Networking

* TCP/IP, DNS, HTTP basics
* Network configuration & troubleshooting
* Learning about distributed systems

### 🔐 Cybersecurity

* Basic system security & hardening
* Linux environments (Parrot OS 🐦)
* Understanding vulnerabilities and protection methods

### ⚙️ Hardware & Systems

* Computer architecture fundamentals
* Embedded systems basics
* Interested in OS development

### 🎨 Web

* HTML, CSS, PHP (learning)

---

## 📌 Current Goals

* 🔹 Build my AI assistant 
* 🔹 Learn advanced cybersecurity & ethical hacking
* 🔹 Design low-level systems (OS / embedded)
* 🔹 Improve networking and system architecture knowledge

---

## ⚡ Mindset

> “Understand the system. Don’t just use it.”

<!--
**Sinshi24/Sinshi24** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
